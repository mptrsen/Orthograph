...
sub new {
	...
	unless (@_ % 2 == 0) { confess("Uneven number of arguments, can't create object\n") }
	my $self = {
		'db'       => undef,
		'query'    => undef,
		'hitcount' => undef,
...
Sets or returns the BLAST database.

Args: scalar string path to BLAST database, or nothing. 

Returns: 1 if called with argument, scalar string path to BLAST database (if set) otherwise.

=cut

sub db {#{{{
	my $self = shift;
	if (@_ > 1) { confess("Called with more than one argument\n") }
	elsif (@_) {
		$self->{'db'} = shift @_;
		return 1;
	}
...
Sets or returns the query.

Args: scalar string query sequence, or scalar string path to query file, or nothing.

Returns: 1 if called with argument, scalar string query (sequence or path to file) otherwise.
...
sub query {
	...
	if (@_ > 1) { confess("Called with more than one argument\n") }
		...
		$self->{'query'} = shift @_;
		# if this is a sequence 'object'
		if ( ref $self->{'query'} and $self->{'query'}->{'header'} and $self->{'query'}->{'sequence'} ) {
			$self->query_file( fastaify($self->{'query'}->{'header'}, $self->{'query'}->{'sequence'}) );
		}
		# if this is a file
		elsif ( -s $self->{'query'} ) {
			$self->query_file($self->{'query'});
		}
		else {
			confess("Neither sequence object nor file\n");
		}
		return 1;
	}
	return $self->{'query'};
...
Sets or returns the query file.
...
Sets or returns the output file.

=cut

sub output_file {
	my $self = shift;
	if (scalar @_ == 0) { return $self->{'outputfile'} }
	else { $self->{'outputfile'} = shift @_ }
}
...
sub blastp {#{{{
	...
	if (-e $outfile) {
		...
		my @blastcmd = qq($searchprog -num_threads $num_threads -outfmt '7 qseqid sseqid evalue bitscore qstart qend' -evalue $evalue_threshold -threshold $score_threshold -max_target_seqs $max_hits -db $db -query $queryfile -out $outfile);
